...
func enter() -> void:
	...
	var instance = gameplay.score_scene.instantiate() as Score
	instance.setup(gameplay.perfect_count, gameplay.good_count, gameplay.early_count, gameplay.late_count)
	gameplay.add_child(instance)

func process(_delta: float) -> int:
	return GameplayState.State.Null
